...
interface IPollDetailContentStates {
    poll:any,
    userReacted:boolean,
...
export default class PollDetailContent extends React.Component<IPollDetailContentProps, IPollDetailContentStates>{

    constructor(props:IPollDetailContentProps) {
        super(props);
        this.state = {
            poll: null,
            user: "",
    ...
    componentWillReceiveProps(prevProps:Readonly<IPollDetailContentProps>, prevState:Readonly<IPollDetailContentStates>, snapshot?:any) {
    ...
    private getReactUsers = (): User[] => {
        const users: User[] = [];
        this.state.poll?.likes && this.state.poll?.likes.map(l => {
            users.push(l)
        })

        this.state.poll?.dislikes && this.state.poll?.dislikes.map(l => {
            users.push(l)
        })
        return users;
    }
            ...
            >
                {
                    ...
                    <View style={{flex:1,width: SCREEN_WIDTH_NEW, height: SCREEN_HEIGHT-60}}>
                            ...
                            <TouchableOpacity onPress={() => {
                                navigate("poll_stats", {poll: this.state.poll,pollCompleted:this.state.remainingTime === "00:00:00"})
                            }}>
                                <View style={[{flexDirection: "row", margin: 5}]}>
                                    ...
                                    <View style={{flexDirection: 'column', justifyContent: 'center'}}>
                                        <Progress.Bar progress={this.getProgressBarValues("like")} width={SCREEN_WIDTH - 50} color={'#82CA5B'} height={20}
                                                      borderRadius={10}/>
                                    </View>
                                </View>
                                <View style={[{flexDirection: "row", margin: 5}]}>
                                    ...
                                    <View style={{flexDirection: 'column', justifyContent: 'center'}}>
                                        ...
                                        <Progress.Bar progress={this.getProgressBarValues("dislike")} width={SCREEN_WIDTH - 50}
                                                      color={'#F7734D'} height={20}
                                                      borderRadius={10}/>
                                    </View>
                            ...
                            <View style={[{flexDirection: "row", marginTop: 10,marginLeft:5}]}>
                                <Text style={{color: '#000', fontWeight: 'bold'}}>{this.state.poll?.description}</Text>
                            </View>
                            <View style={[{flexDirection: "row", margin: 5}]}>
                                <View style={{flexDirection: 'column', justifyContent: 'center'}}>
                                    <Image
